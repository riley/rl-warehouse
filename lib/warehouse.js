    ...
    createSingleOrder: function (order, cb) {
            ...
            '<OrderLineItems>\n',
                ...
                '<% } else { %>',
                    ...
                    '<% _.each(items, function (item) { %>',
                        ...
                        '<SKU><%= item.sku %></SKU>\n',
                        '<Qualifier>None</Qualifier>\n',
                        '<Qty><%= item.quantity * item.initialQuantity %></Qty></OrderLineItem>\n',
                    '<% }); %>',
        ...
        '<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">',
            '<soap:Body>\n',
                '<extLoginData xmlns="http://www.JOI.com/schemas/ViaSub.WMS/">\n',
                    '<ThreePLKey>{9d99620a-e0d4-4bf8-96e4-b8dafa86876e}</ThreePLKey>\n', // this was a number from the docs. unique to us?
                    '<Login>mr</Login>\n',
                    '<Password>davis</Password>\n',
                    '<FacilityID>1</FacilityID>\n',
                '</extLoginData>\n',
        ...
        _request('Create Order', function (err, json) {
            try {
                ...
                return cb(null, processed, order);
            } catch (e) {
                return cb({
